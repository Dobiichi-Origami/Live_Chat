...
func InTestCreateCollectionsAndIndexesAndConnection() error {
	return InitMongoDBConnection(testMongodbConfigFilePath)
}

func TestRegister(t *testing.T) {
	if _, err := Register(nil, mockUserName1, mockEmail, mockPassword); err != nil {
	...
	if _, err := Register(nil, mockUserName2, mockEmail, mockPassword); err != nil {
		t.Fatalf("Mysql register user failed: %s", err.Error())
	}
}
...
func TestLogin(t *testing.T) {
	var err = error(nil)
	mockUserId1, err = Login(nil, mockUserName1, mockPassword)
	if err != nil {
	...
	if _, err = Login(nil, mockBadAccount, mockPassword); err == nil {
		t.Fatalf("Mysql login wrong user found")
	}

	mockUserId2, err = Login(nil, mockUserName2, mockPassword)
	if err != nil {
		t.Fatalf("Mysql login failed: %s", err.Error())
	} else if mockUserId1 == -1 {
		t.Fatalf("Mysql login user not found")
...
func addFriends(id1, id2 int64, t *testing.T) {
	chatId, err := AgreeFriendShip(nil, id1, id2)
	if err != nil {
...
func friendshipCheck(id1, id2, chatId int64, t *testing.T) int64 {
	friendships, err := SelectFriendShip(nil, id1)
	if err != nil {
...
func modifyUsername(id int64, name string, t *testing.T) {
	if err := UpdateUserName(nil, id, name); err != nil {
		t.Fatalf("Mysql modify username failed: %s", err.Error())
	}

	if userInfo, err := SearchUserInfo(nil, id, true); err != nil {
...
func deleteFriendship(id1, id2 int64, t *testing.T) {
	friendships1, err := SelectFriendShip(nil, id1)
	if err != nil {
	...
	if err := DeleteFriendShip(nil, id1, id2); err != nil {
	...
	friendships2, err := SelectFriendShip(nil, id2)
	if err != nil {
		t.Fatalf("Mysql finds user failed: %s", err.Error())
	}
	...
	chatId2, err := AgreeFriendShip(nil, id1, id2)
	if err != nil {
...
func createGroup(owner int64, name, instruction, avatar string, t *testing.T) int64 {
	groupId, err := AddGroupInfo(nil, owner, name, instruction, avatar)
	if err != nil {
		t.Fatalf("Mysql creats new group failed: %s", err.Error())
	}

	_, err = SearchGroupInfo(nil, groupId, true)
	if err != nil {
...
func joinAuthAndQuitGroup(id, groupId int64, t *testing.T) {
	...
	groupMember, err := SelectGroupMemberList(nil, groupId)
	if err != nil {
		t.Fatalf("Mysql finds group by id failed: %s", err.Error())
	}
	...
	if err = DeleteAdministrator(nil, id, groupId); err != nil {
...
func updateGroupInfoAndDeleteGroup(id int64, name, introduction, avatar string, t *testing.T) {
	groupInfo, err := SearchGroupInfo(nil, id, true)
	if err != nil {
		t.Fatalf("Mysql finds group by id failed: %s", err.Error())
	}
	...
	if err = UpdateGroupName(nil, id, name); err != nil {
		t.Fatalf("Mysql group modifies group name failed: %s", err.Error())
	}

	if err = UpdateGroupIntroduction(nil, id, introduction); err != nil {
		t.Fatalf("Mysql group modifies group isntruction failed: %s", err.Error())
	}

	if err = UpdateGroupAvatar(nil, id, avatar); err != nil {
		t.Fatalf("Mysql group modifies group avatar failed: %s", err.Error())
	}

	modifiedInfo, err := SearchGroupInfo(nil, id, true)
	if err != nil {
		t.Fatalf("Mysql finds group by id failed: %s", err.Error())
	}
	...
	if err = DeleteGroupInfo(nil, id); err != nil {
...
func checkChatSeq(chatId int64, t *testing.T) {
	if idSlice, err := GetChatSeqInSlice(context.Background(), []int64{chatId}); err != nil {
...
func checkWatchAndRetrieve(chatId int64, t *testing.T) {
	...
	protoMsg := &rpc.Message{
		Id:        0,
		Sender:    mockUserId1,
		Receiver:  chatId,
		Timestamp: 0,
		Type:      rpc.Message_Text,
		Contents:  []string{"这是一条测试消息"},
	}
	if err = AddMessage(context.Background(), protoMsg); err != nil {
		t.Fatalf("Mongodb add message failed: %s", err.Error())
	}

	if seq, err := GetChatSequence(context.Background(), chatId); err != nil {
