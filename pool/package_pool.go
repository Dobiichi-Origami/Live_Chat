...
// 后接 1 字节的消息体类型
//  0 为 protocol.ErrorResponse,
//  1 为 SuccessResponse, 无对应 Proto，即 Load 中 content 为空
//	2 为 protocol.Message,
//	3 为 protocol.RequestMessage,
//	4 为 protocol.MultiMessage,
//	5 为 protocol.RequestMultiMessage,
//  6 为 protocol.RequestEstablishConnection,
//  7 为 protocol.ResponseEstablishConnection,
//  8 为 HeartBeatLoad, 无对应 Proto
//  9 为 HeartBeatResponse, 无对应 Proto
...
func (p *TCPRequestPackage) SetPackage(c gnet.Conn) error {
...
func PutRequestPackage(ptr *TCPRequestPackage) {
	packagePool.Put(ptr)
}
